...
function futureDateValidator(previousDate: Date | null): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    if (
      ...
      selectedDate.getTime() === (previousDate?.getTime() || 0)
    ) {
...
export class HistoricalExchangeRateComponent implements OnInit {
  ...
  public buttonMessage = 'Get historical exchanges';

  constructor(
    private readonly store: Store<{
      store: { historicalExchangeRates: ExchangeRatesResponse };
    }>,
  ...
  ngOnInit() {
    this.historicalExchangeRateForm = this.formBuilder.group({
      selectedDate: [
        this.chosenDate,
        [Validators.required, futureDateValidator(this.chosenDate)],
      ],
  ...
  public getHistoricalData(event: { date: Date; symbols: string[] }) {
    ...
    this.historicalExchangeRates$ = this.store.select(
      (state) => state.store.historicalExchangeRates
    );
  }
}
