...
export class LatestExchangeRateComponent implements OnInit {
  ...
  constructor(
    private readonly store: Store<{ store: { latestExchangeRates: ExchangeRatesResponse } }>,
  ...
  public getLatestData(event: { date: Date, symbols: string[] }) {
    ...
    this.latestExchangeRates$ = this.store.select(state => state.store.latestExchangeRates);

  }

  public isCurrencySelected(currency: string): boolean {
    const selectedCurrencies = this.latestExchangeRateForm.get('selectedSymbols')?.value;
